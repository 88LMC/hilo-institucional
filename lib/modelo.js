...
const SIGLAS = new Set(['CCSS', 'ICE', 'INA', 'UCR', 'UNED', 'MEP', 'CNP', 'FANAL', 'OIJ', 'SICOP', 'CEN', 'CINAI', 'CEN-CINAI', 'UTGV', 'RECOPE', 'JAPDEVA', 'TEC', 'CTP', 'MOPT', 'AYA', 'INS', 'SST', 'DISA', 'CCDR', 'CCDRG', 'BCR', 'BN', 'JADGME']);
...
export const NUMERO_SICOP = /\d{4}[A-Z]{2,3}-\d{6}-\d{6,12}/;
export function leerAlerta(t) {
  const txt = String(t ?? '').replace(/\r/g, '').replace(/[ \t ]+/g, ' ');
  const g = (re) => { const m = txt.match(re); return m ? m[1].trim() : ''; };
  const numero = (txt.match(NUMERO_SICOP) || [''])[0];
  // Invitación de SICOP: "…le informa que <INSTITUCIÓN> le invita a participar en el concurso <TIPO> No. <NÚMERO> para <OBJETO>."
  const inv = txt.match(/informa que\s+([\s\S]+?)\s+le invita a participar en el concurso\s+([\s\S]+?)\s+N[oº°]\.?\s*\S+\s+para\s+([\s\S]+?)\.?\s*(?:\n|Fecha|Las condiciones|$)/i);
  let institucion = inv ? inv[1].replace(/\s+/g, ' ').trim() : g(/Instituci[oó]n\s*:\s*(.+)/i);
  const sigla = !inv && institucion.match(/\(([^)]+)\)/);
  if (sigla) institucion = sigla[1];
  const objeto = inv ? inv[3].replace(/\s+/g, ' ').trim() : g(/(?:Descripci[oó]n|Objeto)\s*:\s*(.+)/i);
  const tipo = inv ? cap(inv[2].replace(/\s+/g, ' ').trim().toLowerCase()) : '';
  const pres = g(/[₡¢]\s*([\d.,\s]+)/);
  ...
  const fecha_apertura = f(g(/apertura[^:\n]*:\s*([^\n]+)/i));
  const fecha_limite = f(g(/(?:recepci[oó]n de ofertas|l[ií]mite|presentaci[oó]n|presentar)[^:\n]*:\s*([^\n]+)/i)) || fecha_apertura;
  return { numero, institucion, objeto, presupuesto, fecha_apertura, fecha_limite, notas: tipo };
}
